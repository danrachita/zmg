...
*Setup instructions for a gitian build of ZMG using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the ZMG
...
Clone the git repositories for zmg and gitian and then checkout the zmg version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the zmg repository
...
To build ZMG (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the zmg repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/zmg/.git/
...
./bin/gbuild --commit zmg=${COMMIT} --url zmg=${URL} ../zmg/contrib/gitian-descriptors/gitian-osx.yml
```
...
and follow the steps in the build process as normal.
    ...
    gpg: skipped "zmgcoin": secret key not available

When you execute `gsign` you will get an error from GPG, which can be ignored. Copy the resulting `.assert` files
in `gitian.sigs` to your signing machine and do

```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/zmg-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/zmg-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/zmg-build.assert
```
...
[zmg/gitian.sigs](https://github.com/zmg-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
